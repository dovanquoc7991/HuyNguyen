...
import type { QuestionGroup, Test } from "@shared/schemaAdmin";
import { api } from "../lib/api";
import { Editor } from "@tinymce/tinymce-react";
import { tinymceConfig } from "../lib/utils";

const listeningFormSchema = z.object({
    ...
    audio_url: z.string().url("Audio URL is required").optional(),
...
type ListeningFormData = z.infer<typeof listeningFormSchema>;

export default function CreateListeningTest() {
    const [, params] = useRoute("/admin/create-listening-test/:id");
    ...
    const form = useForm<ListeningFormData>({
        resolver: zodResolver(listeningFormSchema),
        defaultValues: {
            title: "",
            time: 30,
    ...
    const { data: existingTest, isLoading: isLoadingTest } = useQuery<Test>({
        ...
        queryFn: () => api.getListeningPartDetail(testId!),
        enabled: isEditing,
    });
    ...
    useEffect(() => {
        if (existingTest) {
            ...
            form.reset({
                ...
                audio_url: existingTest.audio_url || "",
                start_question_number: 1,
    ...
    const createListeningMutation = useMutation({
        mutationFn: (payload: any) => api.createListeningTest(payload),
        onSuccess: () => {
        ...
        onError: () => {
            toast({
                title: "Error",
                description: "Failed to create listening test",
                variant: "destructive",
            });
        },
    });

    // Mutation for updating test
    const updateListeningMutation = useMutation({
        mutationFn: (payload: any) => {
            if (!testId) { // Giữ nguyên kiểm tra testId
                return Promise.reject(new Error("Cannot update test: Existing section ID not found."));
            }
            return api.updateListeningPart(testId, payload); // Giữ nguyên, đã đúng
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["part-list-by-type", "Reading"] });
            queryClient.invalidateQueries({ queryKey: ["part-list-by-type", "Listening"] });
    ...
    // Handle submit
    const onSubmit = (data: ListeningFormData) => {
        if (isEditing) { // Sửa logic cho trường hợp update
            const payload = {
                title: data.title,
                time: String(data.time), // Chuyển time thành chuỗi
            ...
            updateListeningMutation.mutate(payload);
        } else {
            // Logic cho trường hợp create (giữ nguyên)
            const listeningSections = data.test_type === "full_test"
                ? Object.values(parts).map((part, idx) => ({
                    title: data.title + ` - Part ${idx + 1}`, time: String(data.time), audio_url: part.audio_url, part_number: idx + 1, passage: null, groups: part.groups, locked: data.locked, password: data.password, explanation: data.explanation,
                }))
                : [{
                    title: data.title, time: String(data.time), audio_url: data.audio_url, part_number: data.part_number || 1, passage: null, groups: questionGroups, locked: data.locked, password: data.password, isBasic: data.testLevel === 'basic', explanation: data.explanation,
                }];

            const payload = {
                description: data.title,
                listening: listeningSections,
            };
            createListeningMutation.mutate(payload);
        }
    };
    ...
    const readyToPublish =
        ...
        (
            form.watch("test_type") === "full_test"
                ...
                : (form.watch("audio_url") || "").trim() && questionGroups.length > 0 && totalQuestions > 0
        );
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <h2 className="text-2xl font-bold text-gray-900">
                            {isEditing ? "Edit Listening Test" : "Create New IELTS Listening Test"}
                        </h2>
                        <p className="text-gray-600 mt-1">
                            Design and configure your IELTS listening test with multiple question types
                        </p>
                    </div>
                    <div className="flex items-center space-x-4">
                        <Button
                            type="submit"
                            form="test-form-listening"
                            disabled={createListeningMutation.isPending || updateListeningMutation.isPending || !readyToPublish}
            ...
            <div className="p-8">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2 space-y-8">
                        <Form {...form}>
                            <form id="test-form-listening" onSubmit={form.handleSubmit(onSubmit, (errors) => {
                                    ...
                                    <CardContent className="space-y-6">
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            ...
                                            <FormField
                                                ...
                                                render={({ field }) => (
                                                    <FormItem>
                                                        <FormLabel>Listening Time (minutes)</FormLabel>
                                                        <FormControl>
                                                            <Input
                                                                type="number"
                                                                {...field}
                                ...
                                <Card className="mb-8">
                                    <CardHeader>
                                        <CardTitle className="flex items-center">
                                            <i className="fas fa-volume-up text-blue-600 mr-3"></i>
                                            Audio File
                                            {form.watch("test_type") === "full_test" && (
                                    ...
                                    <CardContent>
                                        ...
                                        <div className="space-y-4">
                                            {form.watch("test_type") === "full_test" ? (
                                                <Input
                                                    type="url"
                                                    placeholder="Enter audio file URL for this part"
                                                    value={parts[currentPart].audio_url}
                                                    onChange={e => setParts(prev => ({
                                                        ...prev,
                                                        [currentPart]: { ...prev[currentPart], audio_url: e.target.value }
                                                    }))}
                                                />
                                            ) : (
                                                <FormField
                                            ...
                                            {(form.watch("test_type") === "full_test"
                                                ? parts[currentPart].audio_url
                                                : form.watch("audio_url")
                                            ) && (
                                                    <audio
                                                        controls
                                                        src={form.watch("test_type") === "full_test"
                                                            ? parts[currentPart].audio_url
                                                            : form.watch("audio_url")
                                                        }
                                    ...
                                    <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 max-w-2xl mx-auto">
                                            ...
                                            <Button
                                                ...
                                                variant="outline"
                                                className="p-3 h-auto flex-col"
                                                onClick={() => addQuestionGroup("DROPDOWN")}
                                            >
                                                <i className="fas fa-caret-square-down text-teal-500 mb-2"></i>
                            ...
                            <CardContent className="space-y-3">
                                ...
                                <div className="flex items-center space-x-3">
                                    <i className={`fas fa-${form.watch("test_type") === "full_test"
                                        ? Object.values(parts).some(part => (part?.audio_url || "").trim()) ? "check-circle text-green-500" : "times-circle text-red-500"
                                        : (form.watch("audio_url") || "").trim() ? "check-circle text-green-500" : "times-circle text-red-500"
                                        }`}></i>
                                    <span className="text-sm text-gray-700">Audio file added</span>
                                </div>
                                <div className="flex items-center space-x-3">
                                    <i className={`fas fa-${form.watch("test_type") === "full_test"
                                ...
                                {form.watch("title") &&
                                    (form.watch("test_type") === "full_test"
                                        ? Object.values(parts).some(part => (part?.audio_url || "").trim() && (part?.groups || []).length > 0 && (part?.groups || []).some(group => (group?.questions || []).length > 0))
                                        : (form.watch("audio_url") || "").trim() && questionGroups.length > 0 && totalQuestions > 0
                                    ) && (
